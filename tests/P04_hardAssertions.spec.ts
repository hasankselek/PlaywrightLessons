...
test.describe('Hard Assertions',async()=>{
    ...
    test.skip('Demoblaze Assertion',async({page})=>{ //.skip devre dışı bırakmayı ifade eder
        ...
        //await expect(page.getByText('Place Order').nth(1)).toBeAttached(); // toBeAttached() var mı yok mu kontrol eder
        await expect(page.getByText('Place Order').nth(1)).not.toBeAttached();
    });

    test.skip('TestAutomationPratice',async({page})=>{
        await page.goto('https://testautomationpractice.blogspot.com/');
        await page.locator('#male').check(); // tikleme yapar
        await expect(page.locator('#male')).toBeChecked(); // Locate edilen checkBox öğe tıklanmış halde mi ?
        await expect(page.locator('#name')).toBeEnabled(); // Locate edilen öğenin enable olup olmadığını kontrol eder
        await expect(page.locator('#name')).not.toBeDisabled(); // Locate edilen öğenin disable olup olmadığını kontrol eder
        await expect(page.locator('#name')).toBeEmpty(); // Locate edilen öğenin boş olup olmadığını kontrol eder
        await page.locator('#name').fill('Hasan');
        //await expect(page.locator('#name')).toBeEmpty();
        await expect(page.locator('#name')).toHaveValue('Hasan'); // 'hasan' değerine sahip mi onu kontrol eder
    });
